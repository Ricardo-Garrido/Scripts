...
foreach ($str in $Bootstrap) 
    {
    $content = Get-Content -path $str -Verbose
    $content | foreach {$_ -replace "rDeployRoot", (Get-ItemProperty -Path DS001: -Name UNCPATH).UNCPATH} | Set-Content $str -Verbose
}
...
Set-ItemProperty -Path DS001: -Name Boot.x86.GenerateLiteTouchISO -Value 'True'

...
(
    ...
    [ValidateSet("FILE","RDGW","ADDS","DHCP","RRAS","RDGW","MGMT","DEPL","ADCA","WSUS","SCVM","SCOR","BitLockerAdmin","WEB")]
    $Role
)
...
switch ($Role)
{
    FILE
    {
        ...
        Add-WindowsFeature -Name "FileAndStorage-Services","File-Services","FS-FileServer","FS-Data-Deduplication"  -ErrorAction Stop
        Add-WindowsFeature -Name "FileAndStorage-Services","File-Services","FS-FileServer","FS-Data-Deduplication"  -ErrorAction Stop
        Add-WindowsFeature -Name "FS-DFS-Namespace","FS-DFS-Replication","RSAT-DFS-Mgmt-Con"
    }
    RDGW
    {
        Write-Output "Adding Windows Features for $Role"
